...
class facileFormsInstaller
{
	...
	static function chmodRecursive($path, $filemode = NULL, $dirmode = NULL)
	{
		$ret = TRUE;
		if (is_dir($path)) {
			$dh = opendir($path);
	...
	static function step3($option)
	{
		...
		$gnOldPieces =
			...
			"'gn_page_fin'," .
			"'gn_page4'," .
			"'gn_validate_page_fin'";
		_ff_query(
			"delete from #__facileforms_pieces " .
			"where package = 'GN' and name in ($gnOldPieces)"
		);
