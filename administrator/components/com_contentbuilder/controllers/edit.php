...
class ContentbuilderControllerEdit extends CBController
{
    ...
    function save($apply=false){
        ...
        if(CBRequest::getVar('cb_controller') == 'edit'){
            $link = JRoute::_('index.php?option=com_contentbuilder&title='.CBRequest::getVar('title', '').(CBRequest::getVar('tmpl', '') != '' ? '&tmpl='.CBRequest::getVar('tmpl', '') : '').(CBRequest::getVar('layout', '') != '' ? '&layout='.CBRequest::getVar('layout', '') : '').'&controller=edit&return='.CBRequest::getVar('return','').'&Itemid='.CBRequest::getInt('Itemid',0), false);
        } else if($apply){
