...
class ContentbuilderControllerForms extends CBController
{
    ...
    function orderup() {
        ...
        $model->move(-1);
        $this->setRedirect( JRoute::_('index.php?option=com_contentbuilder&controller=forms&limitstart='.CBRequest::getInt('limitstart'), false) );
    }
    ...
    function saveNew()
    {
        ...
        // Check the table in so it can be edited.... we are done with it anyway
        $link = JRoute::_('index.php?option=com_contentbuilder&controller=forms&task=edit&limitstart='.CBRequest::getInt('limitstart'), false);
        $this->setRedirect($link, $msg);
    }
